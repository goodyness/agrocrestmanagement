...
import { Egg, PawPrint, Plus, ClipboardCheck, History, RefreshCw, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function ExpectedStockTab() {
  ...
  const expectedEggs = useMemo(() => {
    ...
    // Add production after baseline
    production.filter(branchFilter).forEach((p) => {
      const t = new Date(p.created_at || p.date).getTime();
      if (t >= baselineTime) {
        total += toPieces(p.crates || 0, p.pieces || 0);
      }
    });
    // Subtract egg sales after baseline
  ...
  const livestockExpected = useMemo(() => {
    return visibleBatches.map((batch) => {
      ...
      // Subtract mortality after baseline for this batch
      mortality.forEach((m) => {
      ...
      // Subtract livestock sales (best-effort: sales tagged with this batch's species)
      sales.filter(branchFilter).forEach((s) => {
        const t = new Date(s.created_at || s.date).getTime();
      ...
      // Note: new batch intake is itself a separate batch — already shown in the list.
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 items-start sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Set a baseline snapshot, then production/intake adds and sales/mortality subtract live.
          </p>
        </div>
            ...
            <CardContent>
              {expectedEggs === null ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <Stat
                    label="Baseline"
                    value={`${eggBaseline!.crates}c ${eggBaseline!.pieces}p`}
                    small
                  />
                </div>
              )}
                  ...
                  <TableBody>
                    {livestockExpected.map(({ batch, baseline, expected }) => (
                      <TableRow key={batch.id}>
                        ...
                        <TableCell>
                          {expected !== null ? (
                            <span className="font-bold text-lg">{expected}</span>
                          ) : (
                        ...
                        <TableCell>
                          {b.item_type === "eggs" ? (
                            ...
                            <Badge variant="secondary" className="capitalize">
                              {batch?.species || "Livestock"}
                            </Badge>
                          )}
                        </TableCell>
                        <TableCell>
                          {b.item_type === "eggs"
                            ? `${b.crates} crates ${b.pieces} pieces`
                            : `${b.animal_count} animals`}
                        </TableCell>
                  ...
                  {recounts.filter(branchFilter).map((r) => {
                    ...
                    return (
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          {isEgg ? <Badge>Eggs</Badge> : (
                            <Badge variant="secondary" className="capitalize">{batch?.species || "Livestock"}</Badge>
                          )}
                        </TableCell>
                        <TableCell>
                          {isEgg
                            ? `${r.expected_crates}c ${r.expected_pieces}p`
                            : r.expected_animal_count}
                        </TableCell>
                        <TableCell>
                          {isEgg
                            ? `${r.actual_crates}c ${r.actual_pieces}p`
                            : r.actual_animal_count}
                        </TableCell>
                        <TableCell>
                          <div className={`flex items-center gap-2 font-medium ${variance === 0 ? "text-green-600" : "text-destructive"}`}>
...
function NewBaselineDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (itemType === "eggs") {
        payload.crates = parseInt(crates) || 0;
        payload.pieces = parseInt(pieces) || 0;
      } else {
        if (!batchId) {
        ...
        payload.animal_count = parseInt(animals) || 0;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>New Stock Baseline</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving}>{saving ? "Saving..." : "Save Baseline"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function RecountDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (target.type === "eggs") {
        ...
        const actualTotal = toPieces(parseInt(actualCrates) || 0, parseInt(actualPieces) || 0);
        payload.actual_crates = parseInt(actualCrates) || 0;
        payload.actual_pieces = parseInt(actualPieces) || 0;
        ...
        const actual = parseInt(actualAnimals) || 0;
        payload.batch_id = target.batchId;
        payload.actual_animal_count = actual;
        payload.expected_animal_count = expected;
        payload.variance_animals = actual - expected;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Physical Recount — {target.label}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          {target.type === "eggs" ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input type="number" min="0" value={actualPieces} onChange={(e) => setActualPieces(e.target.value)} />
                </div>
              </div>
            </>
          ) : (
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={submit} disabled={saving}>{saving ? "Saving..." : "Save Recount"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
